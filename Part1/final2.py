...
def convert_to_numerical_dbscan(data):
    # Convert categorical variables to numerical variables
    le = LabelEncoder()
    columns_to_encode = ['workclass', 'mariatal_status', 'occupation', 'relationship', 'race', 'sex', 'native_country', 'census_income']
    ...
    # One-hot encoded data
    data_one_hot = data.copy()
    attributes = ['workclass', 'mariatal_status', 'occupation', 'relationship', 'race', 'sex', 'native_country', 'census_income']
    data = pd.get_dummies(data_one_hot, columns=attributes, prefix=attributes)
